...
date: "Week 1: Tuesday, June 4, 2019"
output: 
  html_document:
    toc: true
    toc_float: true
...
```{R, warning = FALSE, message = FALSE}
pirates %>% filter(year >= 1960 & year <= 1990) %>% 
  ggplot(aes(x = final_rank, y = ..count.. / sum(..count..) )) + geom_bar()
```
...
**Question 6**.  We can also use radial graphs to dispaly 1D categorical data.  You are perhaps most familiar with pie charts.  Did you know that a pie chart can be made from a bar plot?
